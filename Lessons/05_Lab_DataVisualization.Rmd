...
# Color your points by year (only will work within aes(),but the location of the statement doesn't matter), and facet by lake in two rows.
# Change the ylab name
ggplot(data = subset(PeterPaul.chem.nutrients,depth==0), aes(x = daynum, y = temperature_C,colour=year4))+
  geom_point()+
...
ggplot(data = PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C,colour=depth))+
  geom_point(size=0.5)+
  ylab("Temperature")+
