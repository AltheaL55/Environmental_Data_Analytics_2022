...
```{r}
#Set the URL
theURL_eno <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data_eno <- read.table(theURL_eno,skip = 28,header=TRUE,sep='\t') 
...
census_api_key("53971290ed5f9019ad47c3ca2b40fc31b3b400d0", install = TRUE,overwrite = T)
...
the_df <- data.frame("states"=the_states, "price" = as.numeric(the_price),"capacity" = as.numeric(gsub(",","",the_capacity)),  "generation" = as.numeric(gsub(",","",net_generation)),"total" = as.numeric(gsub(",","",total_retail)))
