...
ggplot(data = BRA_complete,aes(x = Date,y = Temperature_C))+
  geom_line()+
  geom_smooth(method = lm)
...
BRA.rain.ts <- ts(BRA_complete$Rainfall, start = c(1901,1), frequency = 12)
...
BRA.temp.ts_Decomposed <- stl(BRA.temp.ts,s.window = "periodic")
BRA.rain.ts_Decomposed <- stl(BRA.rain.ts,s.window = "periodic")

plot(stl(BRA.temp.ts,s.window = "periodic"))
plot(stl(BRA.rain.ts,s.window = "periodic"))
...
BRA.temp.ts_Components <- as.data.frame(BRA.temp.ts_Decomposed$time.series[,1:3])
BRA.rain.ts_Components <- as.data.frame(BRA.rain.ts_Decomposed$time.series[,1:3])


# Exercise 10: Visualize how the trend maps onto the data for both series

ggplot(data = BRA.temp.ts_Components)+
