...
```{r}
highTF <- c(45, 48, 35, 43, 52, 51, 60, 66, 59, 45)
lowTF <- c(27, 27, 15, 26, 25, 33, 49, 48, 27, 22)
```
...
```{r}
Tforc <- data.frame(lowTF,highTF,lowTC,highTC)
colnames(Tforc) <- c("low temperature (F)","high temperature (F)","low temperature (C)", "high temperature (C)")
```
...
```{r}
summary(Tforc)
sd(Tforc$`low temperature (F)`)
sd(Tforc$`high temperature (F)`)
```
...
<<<<<<< HEAD
format(today, format = "%m-%d-%Y") #US format
#format(today, format = "")
=======
